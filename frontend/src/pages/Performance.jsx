  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6 bg-[#0B0E14]/90 backdrop-blur-xl p-6 rounded-3xl border border-white/10 shadow-2xl">
        ...
        <button className="flex items-center gap-2 bg-white/5 border border-white/10 px-5 py-2.5 rounded-xl text-xs uppercase tracking-widest font-black text-white shadow-sm hover:bg-white/10 transition-colors">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-[#0B0E14]/90 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col">
          <div className="p-6 border-b border-white/10 bg-white/5">
             ...
             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Based on aggregated historic metrics</p>
          </div>
                ...
                topPerformers.map((student, idx) => (
                   <div key={idx} className="flex justify-between items-center bg-[#11151F] border border-white/5 px-4 py-3 rounded-2xl hover:border-white/10 transition-colors">
                      ...
                      <div className="flex flex-col items-end">
                         ...
                         <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500 bg-white/5 px-2 py-0.5 rounded border border-white/5 mt-1">{student.grade}</span>
                      </div>
                   </div>
                ))
             ) : (
                <div className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-white/10 rounded-2xl w-full h-full bg-white/5 shadow-inner">
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-xs text-center">No historic performance data recorded yet</p>
                </div>
             )}
          </div>
        </div>
