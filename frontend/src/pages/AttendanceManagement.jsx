...
export default function AttendanceManagement() {
  ...
  const [date, setDate] = useState(() => {
    const now = new Date();
    // Start of current day in local for the picker
  ...
  // Fetch Users and potential existing Attendance when Course, Section, or Date changes
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        // Our new isolated backend routes
        ...
        if (existingDoc && existingDoc.records && existingDoc.records.length > 0) {
            ...
            // First time taking attendance for this specific day
            initialRecords = [
                 ...
                 <div key={userItem.userId} className={`flex flex-col bg-[#11151F]/5 border border-white/5 p-4 rounded-3xl backdrop-blur-md transition-all duration-300 shadow-md ${
                    ...
                    <div className="flex bg-black/40 rounded-xl p-1 gap-1 w-full mt-auto border border-white/5">
                        ...
                        <label className={`flex-1 flex justify-center items-center gap-1.5 py-2 rounded-lg cursor-pointer transition-all duration-200 ${
                           ...
                           <span className="text-xs font-black uppercase">Late</span>
                        </label>
  ...
  return (
    <div className="animate-in fade-in flex flex-col space-y-8 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-white/10 pb-6 pt-2">
        <div>
          <h1 className="text-4xl font-display font-black text-white tracking-wider flex items-center gap-3">
        ...
        {message && (
          <div className="px-6 py-2.5 rounded-2xl font-bold text-sm bg-[#008A32]/10 border border-[#008A32]/30 text-[#008A32] shadow-lg animate-in slide-in-from-right">
             {message}
          </div>
        )}
      </div>

      <div className="bg-[#0B0E14]/70 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-2xl flex flex-col md:flex-row gap-6 mt-4 relative overflow-hidden">
         ...
         <div className="w-full md:w-[30%]">
            ...
            <div className="relative">
              <input 
                 ...
                 className="w-full pl-12 pr-4 py-3 bg-black/40 border border-white/10 text-white rounded-xl text-sm font-bold focus:outline-none focus:border-[#FFD700] hover:bg-black/60 transition-colors"
              />
         ...
         <div className="space-y-12 animate-in slide-in-from-bottom-6 duration-700 pb-12">
            ...
            <div className="sticky bottom-6 flex justify-end z-40">
               <button 
                  onClick={handleSave}
                  disabled={saving}
                  className="bg-gradient-to-r from-[#FFD700] to-[#E5C100] hover:shadow-[0_0_30px_rgba(255,215,0,0.4)] hover:-translate-y-1 text-black px-10 py-4 rounded-2xl font-black uppercase tracking-widest transition-all disabled:opacity-50 disabled:hover:translate-y-0 flex items-center items-center gap-3"
               >
