  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto text-slate-200">
      <div className="mb-10">
        ...
        <p className="text-slate-200 font-medium">Create groupings, assign instructors, and organize students tightly into cohorts.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-[#11151F]/40 backdrop-blur-xl border border-white/5 shadow-2xl rounded-[2.5rem] p-6 lg:p-8 relative overflow-hidden">
               ...
               <form onSubmit={handleCreateSection} className="space-y-5">
                 <div>
                   <label className="block text-xs font-bold text-slate-200 uppercase tracking-widest mb-2">Section Name</label>
                 ...
                 <div>
                   <label className="block text-xs font-bold text-slate-200 uppercase tracking-widest mb-2">Target Course</label>
                   <div className="relative">
                     ...
                     <BookOpen className="w-4 h-4 text-slate-300 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" />
                   </div>
                 </div>

                 <div>
                   <label className="block text-xs font-bold text-slate-200 uppercase tracking-widest mb-2">Assign Instructor</label>
                   <div className="relative">
                     ...
                     <ShieldAlert className="w-4 h-4 text-slate-300 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" />
                   </div>
                 </div>
            ...
            {selectedSection && (
              <div className="bg-[#11151F]/40 backdrop-blur-xl border border-white/5 shadow-2xl rounded-[2.5rem] p-6 lg:p-8 animate-fade-in-up">
                 ...
                 <form onSubmit={handleAddStudent} className="flex flex-col gap-3 mb-6">
                   <label className="text-xs font-bold text-slate-200 uppercase tracking-widest">Enroll New Learner</label>
                 ...
                 <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 scrollbar-hide border-t border-white/5 pt-4">
                   <p className="text-xs font-bold text-slate-300 tracking-wider mb-3">{selectedSection.students.length} Learners Enrolled</p>
                   {selectedSection.students.map(student => (
                     <div key={student.id} className="flex items-center justify-between bg-[#0B0E14]/60 border border-white/5 p-3 rounded-xl hover:border-white/10 transition-colors">
                       <div className="flex items-center gap-3">
                         ...
                         <div className="flex flex-col">
                           ...
                           <span className="text-[10px] text-slate-200">{student.email}</span>
                         </div>
                       </div>
                   ...
                   {selectedSection.students.length === 0 && (
                     <div className="text-center text-slate-300 text-sm py-4 italic">No students linked to this array yet.</div>
                   )}
                 </div>
              </div>
            )}
         ...
         <div className="lg:col-span-2 space-y-6">
           ...
           {courses.map(course => {
             ...
             return (
               <div key={course.id} className="bg-[#11151F]/30 border border-white/5 rounded-3xl p-5 md:p-6 mb-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {courseSections.map(sec => (
                     <div 
                       ...
                       className={`p-5 rounded-2xl border transition-all cursor-pointer group relative overflow-hidden ${selectedSection?.id === sec.id ? 'bg-[#008A32]/10 border-[#008A32]/40 shadow-[0_0_20px_rgba(0,138,50,0.1)]' : 'bg-[#0B0E14]/60 border-white/5 hover:border-white/10 hover:bg-[#0B0E14]'}`}
                     >
                        <div className="flex justify-between items-start mb-3">
                           <div>
                             <h5 className="font-bold text-white text-base group-hover:text-[#FFD700] transition-colors">{sec.name}</h5>
                             <p className="text-[10px] text-slate-200 font-bold uppercase tracking-widest mt-1">
                               Instructor: {sec.instructor ? sec.instructor.name : <span className="text-rose-400">Unassigned</span>}
                             </p>
                           </div>
                           <button onClick={(e) => { e.stopPropagation(); handleDeleteSection(sec.id); }} className="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all bg-[#11151F]/5 hover:bg-rose-500/100/10 p-1.5 rounded-lg border border-transparent hover:border-rose-500/30">
                              <Trash2 className="w-4 h-4" />
                           </button>
                        </div>
           ...
           {sections.length === 0 && (
              <div className="bg-[#11151F]/30 border border-white/10 rounded-3xl p-16 text-center text-slate-200 font-medium italic">
