...
export default function UserAvatar({ user, className = "w-8 h-8 md:w-9 md:h-9 text-xs md:text-sm" }) {
  ...
  if (hasAvatar) {
    ...
    return (
      <div className={`rounded-full overflow-hidden shrink-0 border border-white/10 dark:border-slate-700 shadow-sm ${className}`}>
        <img 
          src={avatarUrl} 
        ...
        <div className="w-full h-full hidden items-center justify-center font-bold bg-indigo-100 text-indigo-600 dark:bg-indigo-500/100/20 dark:text-indigo-400 uppercase">
