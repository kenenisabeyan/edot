...
export default function ActivityFeed({ isAdmin = false, feedType, limit = 5 }) {
  ...
  const getBgColorForType = (type) => {
    switch (type) {
      ...
      case 'system': return 'bg-slate-500/10 border-white/5';
  ...
  if (activities.length === 0) {
    return (
      <div className="bg-[#0B0E14]/40 rounded-2xl border border-white/5 p-8 text-center flex flex-col items-center">
        ...
        <p className="text-xs text-slate-500 mt-1">Activities will appear here once actions are taken.</p>
      </div>
    );
  }
      ...
      {activities.map((activity, index) => (
        <div key={activity._id || index} className="flex gap-4 items-start relative group">
          {index !== activities.length - 1 && (
            <div className="absolute left-6 top-10 bottom-[-1rem] w-px bg-white/5 -z-10 group-hover:bg-[#FFD700]/20 transition-colors"></div>
          )}
          ...
          <div className="flex-1 bg-[#11151F]/80 p-4 rounded-2xl border border-white/5 shadow-lg shadow-black/20 hover:border-white/10 transition-colors backdrop-blur-sm">
            <div className="flex justify-between items-start gap-2 mb-2">
              ...
              <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider shrink-0 whitespace-nowrap bg-white/5 px-2.5 py-1 rounded-md border border-white/5">
            ...
            {isAdmin && activity.user && (
              <p className="text-xs text-slate-400 mb-2 font-medium">
                <span className="text-slate-500 font-bold uppercase tracking-widest text-[9px]">User:</span> {activity.user.name} <span className="text-[9px] font-black tracking-widest uppercase bg-[#FFD700]/10 border border-[#FFD700]/20 text-[#FFD700] px-1.5 py-0.5 rounded ml-1">{activity.user.role}</span>
              </p>
            )}
            {activity.details && (
            ...
            <div className="flex items-center justify-between mt-4">
               <div className="flex items-center gap-2">
                 {activity.visibility === 'private' && <span className="text-[9px] uppercase font-black tracking-widest text-slate-500 bg-white/5 border border-white/10 px-2 py-1 rounded-md shadow-sm">Private</span>}
                 {activity.visibility === 'insight' && (
                   <span className={`text-[9px] uppercase font-black tracking-widest px-2 py-1 rounded-md shadow-sm flex items-center gap-1 border ${activity.insightFlag === 'achievement' ? 'text-[#008A32] bg-[#008A32]/10 border-[#008A32]/20' : activity.insightFlag === 'concern' ? 'text-[#E30A17] bg-[#E30A17]/10 border-[#E30A17]/20' : 'text-blue-400 bg-blue-500/10 border-blue-500/20'}`}>
               ...
               {(user?.role === 'instructor' || user?.role === 'admin') && filterType === 'all' && activity.visibility !== 'insight' && (
                 <div className="flex gap-2">
                   <button onClick={() => handleFlag(activity._id, 'achievement')} className="text-[10px] font-black uppercase tracking-widest text-[#008A32] hover:bg-[#008A32]/20 bg-[#008A32]/10 border border-[#008A32]/20 px-3 py-1 rounded-md transition-colors">Flag Achievement</button>
                   <button onClick={() => handleFlag(activity._id, 'concern')} className="text-[10px] font-black uppercase tracking-widest text-[#E30A17] hover:bg-[#E30A17]/20 bg-[#E30A17]/10 border border-[#E30A17]/20 px-3 py-1 rounded-md transition-colors">Flag Concern</button>
                 </div>
               )}
