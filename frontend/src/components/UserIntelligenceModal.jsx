        ...
        >
          {loading ? (
            ...
            <motion.div
              ...
              className="relative w-full max-w-6xl rounded-3xl border border-[#FFD700] bg-[#11151F]/40 p-5 md:p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-2xl max-h-[85vh] overflow-y-auto custom-scrollbar"
            ...
            <div className="flex justify-between items-start gap-4 mb-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold uppercase tracking-wider text-slate-200">{selectedUser.role || 'Unknown Role'}</p>
              ...
              <button onClick={onClose} className="px-4 py-2 rounded-lg bg-[#11151F]/5 text-slate-300 text-sm font-bold hover:bg-[#11151F]/10 hover:text-white transition-colors">Close Detial</button>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5 shrink-0">
              {/* Relationship Map */}
              <div className="p-5 rounded-2xl border border-white/5 bg-[#11151F]/5">
                <h4 className="text-xs text-slate-200 font-bold uppercase tracking-widest mb-4">Relationship Map</h4>
                {selectedUser.role === 'student' && (
                  <div className="space-y-4">
                    <div className="pb-2">
                      <p className="text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-1">Assigned Instructor</p>
                      {selectedUser.assignedInstructor ? (
                        <span className="text-sm font-semibold text-emerald-300">
                          {selectedUser.assignedInstructor.name}
                        </span>
                      ) : (
                        <p className="text-sm text-slate-300 italic">Unassigned</p>
                      )}
                    </div>
                    <div className="pt-2 border-t border-white/5">
                      <p className="text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-2">Change Instructor</p>
                      <div className="flex gap-2">
                        <CustomDropdown
                          ...
                          className="flex-1"
                        />
                        <button onClick={() => updateStudentInstructor(instructorToAssign)} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500/100 rounded-xl text-[10px] font-bold text-white uppercase tracking-wider transition-colors">Set</button>
                      </div>
                    </div>
                    <div className="pt-3 border-t border-white/5">
                      <p className="text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-2">Linked Parents</p>
                        ...
                        {(selectedUser.parents || []).length > 0 ?
                          selectedUser.parents.map((parent) => (
                            <div key={parent.id} className="flex items-center gap-1 px-3 py-1.5 text-[11px] font-bold text-[#0B0E14] bg-[#FFD700] rounded-lg shadow-sm">
                              ...
                              <button onClick={() => untieParentFromStudent(parent.id)} className="ml-1 text-[#0B0E14]/60 hover:text-red-700 text-[10px] uppercase font-black transition-colors">X</button>
                            </div>
                          )) : <p className="text-sm text-slate-300 italic">No parents connected</p>
                        }
                      </div>
                      <div className="flex gap-2">
                        <CustomDropdown
                          ...
                          className="flex-1"
                        />
                        <button onClick={() => bindParentToStudent(parentToBind)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500/100 rounded-xl text-[10px] font-bold text-white uppercase tracking-wider transition-colors">Add</button>
                      </div>
                    </div>
                  </div>
                )}
                {selectedUser.role === 'instructor' && (
                  <div className="space-y-2">
                    <p className="text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-1">Active Students Mentored</p>
                    <p className="text-2xl font-black text-emerald-300">{selectedUser.assignedStudents?.length || 0}</p>
                  </div>
                )}
                {selectedUser.role === 'parent' && (
                  <div className="space-y-3">
                    <p className="text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-1">Watching Learners</p>
                    {(selectedUser.children || []).length > 0 ?
                      ...
                      )) : <p className="text-sm text-slate-300 italic">No current children linked</p>
                    }
                  </div>
                )}
                {selectedUser.role === 'admin' && (
                  <p className="text-sm text-slate-300 italic">System Owner</p>
                )}
              </div>

              {/* Activity Matrix */}
              <div className="p-5 rounded-2xl border border-white/5 bg-[#11151F]/5">
                <h4 className="text-xs text-slate-200 font-bold uppercase tracking-widest mb-4">Activity Matrix (Recent)</h4>
                <div className="max-h-48 overflow-auto space-y-2 custom-scrollbar pr-2">
                  {(selectedUserActivities.length > 0) ? selectedUserActivities.map((activity) => (
                    <div key={activity.id} className="rounded-xl border border-slate-800 bg-black/40 p-3 text-xs shadow-inner">
                      <div className="flex justify-between items-start">
                         ...
                         {activity.metadata?.ip && <span className="text-[9px] text-slate-300 font-mono tracking-tighter bg-[#11151F]/5 border border-white/10 px-1.5 py-0.5 rounded">{activity.metadata.ip}</span>}
                      </div>
                  ...
                  )) : <p className="text-slate-300 text-sm italic">No recent activity recorded.</p>}
                </div>
              </div>

              {/* Extended Insights */}
              <div className="p-5 rounded-2xl border border-white/5 bg-[#11151F]/5">
                <h4 className="text-xs text-slate-200 font-bold uppercase tracking-widest mb-4">Extended Insights</h4>
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    <p className="text-[10px] text-slate-300 uppercase font-bold tracking-wider mb-1">Last online</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5 shrink-0">
              ...
              <div className="p-5 rounded-2xl border border-white/5 bg-[#11151F]/5 flex flex-col items-center justify-center relative overflow-hidden">
                <h4 className="absolute top-5 left-5 text-xs text-slate-200 font-bold uppercase tracking-widest">Performance Snapshot</h4>
                ...
                <p className="text-center mt-3 text-xs text-slate-200 font-bold tracking-widest uppercase">Global Average Progress</p>
              </div>
              ...
              <div className="p-5 rounded-2xl border border-white/5 bg-[#11151F]/5">
                {(selectedUser.role === 'student') ? (
                  <>
                    <h4 className="text-xs text-slate-200 font-bold uppercase tracking-widest mb-4">Student Dossier</h4>
                        ...
                        return (
                          <div key={`${courseId}_${idx}`} className="rounded-xl border border-slate-800 bg-black/40 p-3 flex flex-col gap-2 relative group overflow-hidden">
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <button onClick={() => removeEnrollment(courseId)} className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] text-rose-300 font-bold tracking-wider uppercase px-2 py-1 rounded bg-rose-500/100/10 hover:bg-rose-500/100/20">Drop</button>
                            </div>
                            ...
                            <p className="text-[10px] font-bold tracking-widest uppercase text-slate-300">Progress: <span className="text-white">{en.progress || 0}%</span> • {en.status}</p>
                          </div>
                        );
                      })}
                    </div>
                    <div className="mt-4 flex flex-col gap-3 pt-4 border-t border-white/5">
                            ...
                            render: (
                              <div className="flex items-center gap-3 w-full py-0.5">
                                ...
                                <div className="flex flex-col text-left flex-1 min-w-0">
                                  ...
                                  <span className="text-[9px] text-slate-200 capitalize flex items-center gap-1.5 mt-0.5">
                      ...
                      <div className="flex flex-wrap gap-2 pt-2 border-t border-white/5">
                        <button onClick={resetUserProgress} className="px-4 py-2 font-bold uppercase tracking-wider bg-amber-500/100/20 text-amber-300 border border-amber-500/30 hover:bg-amber-500/100/40 rounded-xl text-[10px] transition-colors">Reset Progress</button>
                        {selectedUser.status === 'blocked' ? (
                          <button onClick={async () => await updateUserStatus('approved')} className="px-4 py-2 font-bold uppercase tracking-wider bg-emerald-500/100/20 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500/100/40 rounded-xl text-[10px] transition-colors">Unblock Service</button>
                        ) : (
                  ...
                  <>
                    <h4 className="text-xs text-slate-200 font-bold uppercase tracking-widest mb-4">Instructor Performance</h4>
                    <div className="space-y-4">
                      <div>
                        <p className="text-[10px] uppercase font-bold tracking-wider text-slate-300 mb-2">Content Portfolio</p>
                        {(selectedUserCourses.length === 0) ? <p className="text-xs text-slate-200 italic">No created content yet</p> : 
                      ...
                      <div className="pt-4 border-t border-white/5">
                        <button onClick={toggleInstructorAccess} className={`w-full py-3 rounded-xl text-xs font-bold uppercase tracking-wider border transition-colors ${selectedUser.status === 'approved' ? 'bg-amber-500/100/10 text-amber-400 border-amber-500/20 hover:bg-amber-500/100/20' : 'bg-emerald-500/100/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/100/20'}`}>
                  ...
                  <>
                    <h4 className="text-xs text-slate-200 font-bold uppercase tracking-widest mb-4">Guardian Insight Management</h4>
                    
                    <div className="space-y-3 max-h-32 overflow-auto custom-scrollbar">
                      {(selectedUser.children || []).length > 0 ?
                        selectedUser.children.map((c) => (
                          <div key={c.id} className="flex items-center justify-between px-4 py-2 bg-black/40 border border-white/5 rounded-xl">
                             <span className="text-sm font-bold text-white">{c.name}</span>
                             <button onClick={() => removeChildFromParent(c.id)} className="text-[10px] px-3 py-1.5 rounded-md font-bold uppercase tracking-wider bg-rose-600/20 text-rose-300 hover:bg-rose-600/40 transition-colors">Untie</button>
                          </div>
                        )) : <p className="text-xs text-slate-300 italic">No learners monitored</p>
                      }
                    </div>

                    <div className="mt-4 pt-4 border-t border-white/5">
                      <p className="text-[10px] uppercase tracking-wider font-bold text-slate-300 mb-2">Bind New Learner</p>
                      <div className="flex gap-2">
                        <input 
                          type="text" 
                        ...
                        <CustomDropdown
                          ...
                          className="flex-1"
                        />
                        <button onClick={() => { if(selectedChildId) addChildToParent(selectedChildId); }} className="px-4 py-2 bg-blue-600 hover:bg-blue-500/100 rounded-xl text-xs font-bold text-white transition-colors">Bind</button>
                      </div>
                    </div>
                  </>
                ) : (
            ...
            {/* Overrides block */}
            <div className="p-5 rounded-2xl border border-white/5 bg-[#11151F]/5 shrink-0">
              <h4 className="text-xs text-slate-200 font-bold uppercase tracking-widest mb-4">Quick Admin Overrides</h4>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-300 mb-1 block">Email Override</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-300 mb-1 block">Role Override</label>
                  <CustomDropdown
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-300 mb-1 block">Force Reset Password</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-300 mb-1 block">Status Force</label>
                  <CustomDropdown
              ...
              <div className="mt-5 pt-5 border-t border-white/5 flex justify-between items-center gap-3">
                <button onClick={saveUserUpdates} className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500/100 text-white font-bold uppercase tracking-widest text-[11px] transition-colors shadow-lg shadow-emerald-600/20">Commit Changes</button>
                <button onClick={deleteAdminUser} className="px-6 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500/100 text-white font-bold uppercase tracking-widest text-[11px] transition-colors shadow-lg shadow-rose-600/20">Purge Data</button>
              </div>
            </div>
            ...
            <div className="p-8 text-center bg-black/80 rounded-2xl border border-white/10 mt-20" onClick={(e) => e.stopPropagation()}>
               ...
               <button onClick={onClose} className="mt-4 px-4 py-2 bg-[#11151F]/10 rounded-lg text-sm text-slate-300">Close</button>
            </div>
          )}
        </motion.div>
      )}
