...
export default function AgendaWidget({ events, userRole, isAdmin, onDelete, onCreateClick }) {
  ...
  const getCategoryColor = (type) => {
    switch(type) {
      ...
      case 'advice': return 'bg-amber-500/20 text-amber-400 border-amber-500/30';
      ...
      <div className="flex flex-col h-full bg-[#11151F]/40 backdrop-blur-xl border border-white/5 rounded-2xl p-5 shadow-lg relative min-h-[350px]">
        ...
        <div className="flex justify-between items-start mb-5 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">{filteredEvents.length} items scheduled</p>
          </div>
                   ...
                   <div className="flex justify-between items-start gap-3">
                     <div className="flex-1">
                       <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                          ...
                          {isInstructorPost && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-md bg-purple-500/20 text-purple-300 border border-purple-500/30 flex items-center gap-1 font-bold">
                     ...
                     <div className="flex flex-col items-end gap-1">
                       ...
                       {(isAdmin || evt.createdBy === 'self' || evt.canDelete) && onDelete && (
                         <button 
                           onClick={(e) => { e.stopPropagation(); onDelete(evt._id); }} 
                           className="text-[10px] bg-rose-500/10 text-rose-400 px-2 py-1 rounded hover:bg-rose-500/20 hover:text-rose-300 transition-colors opacity-0 group-hover:opacity-100"
                         >
                           Delete
                         </button>
                       )}
            ...
            >
               ...
               <div className="px-6 py-5 border-b border-white/5 flex justify-between items-center bg-[#0B0E14]/50">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">{new Date(selectedEvent.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} at {selectedEvent.time || '12:00 PM'}</p>
                    </div>
                 </div>
                 <button onClick={() => setSelectedEvent(null)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 text-slate-400 hover:text-white transition-colors">
                   <X className="w-4 h-4" />
                 </button>
               </div>
               ...
               <div className="p-6">
                 <div className="min-h-[100px]">
                    <h5 className="text-xs text-slate-500 uppercase tracking-widest font-bold mb-3 border-b border-white/5 pb-2">Full Details</h5>
               ...
               <div className="p-5 border-t border-white/5 bg-[#0B0E14]/40 flex justify-end">
                 <button 
                   onClick={() => setSelectedEvent(null)}
                   className="flex items-center gap-2 px-6 py-2.5 bg-emerald-500/10 text-emerald-400 font-bold rounded-xl border border-emerald-500/20 hover:bg-emerald-500 hover:text-white transition-all shadow-[0_0_15px_rgba(16,185,129,0.1)] hover:shadow-[0_0_20px_rgba(16,185,129,0.3)]"
                 >
